...
export function MediaPreviewModal({ item, previewId, onClose }: Props) {
  ...
  return (
    <Portal forceMount>
      ...
      <Content asChild forceMount onInteractOutside={e => e.preventDefault()}>
        <motion.div
        ...
        >
          <Title className="sr-only">{item.title}</Title>
          ...
          <Close asChild>
            <motion.button
              ...
              className="absolute right-4 top-4 z-10 flex size-10 items-center justify-center rounded-full bg-white/15 backdrop-blur-md transition-colors hover:bg-white/25 lg:hidden"
              aria-label="Close"
            >
